...
BEGIN
    ...
    END IF;
    IF (SELECT count(*) FROM cost_manual_snapshot.snapshot_count WHERE batch_code = v_batch AND tenant_id = v_tenant) <> 5 THEN
    ...
    FROM (
        SELECT expected.object_name, expected.row_count AS expected_count,
               CASE expected.object_name
                   ...
                   WHEN 'warning_state' THEN (SELECT count(*) FROM "costree_mvp".cost_warning_record WHERE tenant_id = v_tenant)
