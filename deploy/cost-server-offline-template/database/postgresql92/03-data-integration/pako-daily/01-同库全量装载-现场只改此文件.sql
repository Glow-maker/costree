-- PAKO_DAILY_MAPPING_TEMPLATE
-- 帕科同库全量装载：现场每天执行前，只需完成一次六段源字段映射。
-- 只允许修改下方六段 SELECT/FROM 中的 <源系统schema>、源表和源字段表达式。
-- 不要修改 INSERT 目标列、事务、空快照保护、READY 标记和最终行数输出。
-- 本文件只写 cost_sync_stage 中间表，不直接写 costree_mvp 业务表。
-- 执行前必须先运行 snapshot-upsert/02-只清空中间表.sql。
