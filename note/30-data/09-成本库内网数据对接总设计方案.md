...
执行入口：需要直接准备测试源表、同步两个演示项目并完成页面对账时，先看 `10-成本库数据源与测试导入闭环.md`。本文保留完整设计背景和字段说明，10 文档负责实际操作。

当前已落地的金额权威口径如下，后文早期字段说明如有冲突，以本段和 10 文档为准：

- 合同、到款、目标、审定来自预分预控“项目 + 核算单位”累计金额，落入 `cost_unit_cost_detail`。
- 账面来自 `cost_work_order_ledger_detail`，源金额 `je` 为元，`amount_wan=je/10000`，只统计借方。
- `cost_work_order.book_cost_amount` 是按年度重建的工作令借方账面快照，不是人工填报金额。
- 项目和工作令中的历史合同、目标字段只保留兼容，不再作为首页和成本树金额来源。
...
| 项目填报 | `cost_project_basic` | 项目办维护阶段、周期、用户等非财务基本情况 | 这些不是主业项目树原始字段，应独立维护 | 项目办填报/导入，金额字段不作为权威来源 |
| 工作令业务 | `cost_work_order` | 年度工作令、阶段、分系统和可重建账面快照 | 映射表说明归属，补充表维护非财务字段，账面由明细重建 | 工作令映射 + 年度账面 + 研制单位补充 |
| 账面明细 | `cost_work_order_ledger_detail` | 账面成本、超支预警、八项科目组成 | 原表金额/日期都是字符串，需标准化后聚合 | 从凭证明细导入解析，只统计借方 |
| 单位金额 | `cost_unit_cost_detail` | 项目单位合同、到款、目标、审定累计值 | 统一承接预分预控治理结果 | 按项目编码 + 核算单位幂等同步 |
...
| `cost_work_order` | 工作令映射 + 年度账面 + 补充字段 | `project_code`、`unit_name`、`work_order_no`、`fiscal_year` | 工作令卡片/列表、阶段、分系统、年度账面 | 同编号跨年分行保存，账面快照只由借方明细重建 |
...
| `cost_project_basic` | 项目办填报/导入 | `project_id/project_code` | 填报页、项目基本信息 | 只维护非财务字段；历史金额字段保留兼容 |
| `cost_unit_cost_detail` | 预分预控治理结果 | `project_code/unit_name` | 首页、成本树合同/到款/目标/审定 | 累计值统一写 `stage_code='ALL'`，不按阶段累加 |
