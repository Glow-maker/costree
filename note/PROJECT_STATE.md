...
0. 自主学习和接手开发时，先阅读 `00-overview/03-成本树代码阅读地图与自学路线.md`，再按 `/cost/catalog` 这条链路练习从页面追到接口、后端和数据库。
1. 重启 `cost-server` 和前端 dev server，使用 `VITE_BASE_URL='http://127.0.0.1:38080'`、不启用 `VITE_COST_MOCK_LOGIN`，确认成本库页面走真实业务中台登录态。
2. 登录业务中台后打开 `/cost/pending-allocation`，复核待分配池指标、4 条测试样本、主表分页和单位抽屉穿透。
3. 打开 `/cost/warning`，复核 SQL 分页后的预警数量、筛选、重新推送和标记已处理。
4. 打开 `/cost/catalog`，复核目录树、列表/卡片切换、工作令后端分页和滚动体验。
5. 若页面仍停留加载态，优先看浏览器 Console 的 `应用初始化失败` 日志和 Network 中 `/admin-api/...` 的状态码。
6. 链路稳定后，迁移原型 `DataAnalysis.tsx` 为 `/cost/analysis`，先做统计概览和 Excel 导出入口。
